...
const STATUS_CFG: Record<string, { label: string; color: string; bg: string; dot: string }> = {
  paid:       { label: 'Pagado',     color: '#2c46cf', bg: '#f4f6fb', dot: '#4a68f5' },
  pending:    { label: 'Pendiente',  color: '#92400e', bg: '#fffbeb', dot: '#f59e0b' },
  failed:     { label: 'Vencido',    color: '#991b1b', bg: '#fef2f2', dot: '#ef4444' },
  processing: { label: 'En revisión',color: '#2c46cf', bg: '#eef2ff', dot: '#4a68f5' },
}

const SINPE_NUMBER = '7260-4278'
const SINPE_NAME = 'GolPlay S.A.'
...
.logo{font-size:22px;font-weight:800;margin-bottom:32px;color:#3a5bf0}.hdr{display:flex;justify-content:space-between;margin-bottom:28px;padding-bottom:20px;border-bottom:2px solid #f1f5f9}
.title{font-size:24px;font-weight:700}.sub{font-size:12px;color:#64748b;margin-top:4px}.badge{display:inline-block;background:#f4f6fb;border:1px solid #dfe6ff;color:#2c46cf;padding:4px 12px;border-radius:999px;font-size:11px;font-weight:700;margin-top:6px}
    ...
    <AdminLayout>
      ...
      <div className="py">
        ...
        <div className="py-kpis">
          {[
            { value: kpis.pendingCount > 0 ? fMoneyShort(kpis.totalPending) : '✓ Al día', label: 'Balance pendiente', accent: kpis.pendingCount > 0 ? '#ef4444' : '#3a5bf0' },
            { value: kpis.nextDue ? relativeDate(kpis.nextDue.due_date) : '—', label: 'Próximo vencimiento', accent: '#f59e0b' },
            { value: String(kpis.paidCount), label: 'Pagos realizados', accent: '#3a5bf0' },
            { value: fMoneyShort(kpis.totalPaid), label: 'Total pagado', accent: '#3a5bf0' },
          ].map((k, i) => (
...
.py-btn--green{background:#3a5bf0;color:#fff;box-shadow:0 2px 8px rgba(58,91,240,.25)}.py-btn--green:hover{background:#2c46cf}
.py-btn--sm{padding:7px 12px;font-size:11px}
.py-btn:disabled{opacity:.45;cursor:not-allowed}

/* Plan card */
.py-plan{display:flex;align-items:center;justify-content:space-between;background:#fff;border:1.5px solid #dfe6ff;border-radius:14px;padding:16px 20px;margin-bottom:16px}
.py-plan__left{display:flex;align-items:center;gap:12px}
.py-plan__icon{width:40px;height:40px;border-radius:10px;background:linear-gradient(135deg,#3a5bf0,#2c46cf);display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0}
...
.py-plan__badge{font-size:11px;font-weight:700;color:#2c46cf;background:#f4f6fb;border:1px solid #dfe6ff;padding:4px 12px;border-radius:999px}
...
.py-sinpe__value--bold{font-size:16px;font-weight:800;color:#3a5bf0}
...
.py-stmt__processing{font-size:11px;font-weight:600;color:#2c46cf;background:#eef2ff;padding:4px 10px;border-radius:8px}
...
.py-badge--paid{background:#f4f6fb;color:#2c46cf}
.py-badge--pending{background:#fffbeb;color:#92400e}
.py-badge--failed{background:#fef2f2;color:#991b1b}
.py-badge--processing{background:#eef2ff;color:#2c46cf}
...
.py-modal__head-icon{width:40px;height:40px;border-radius:10px;background:#f4f6fb;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0}
...
.py-method:hover{border-color:#3a5bf0;color:#2c46cf}
.py-method--sel{border-color:#3a5bf0;background:#f4f6fb;color:#2c46cf}
...
.py-upload:hover{border-color:#3a5bf0;background:#f4f6fb}
.py-upload--has{border-color:#3a5bf0;background:#f4f6fb;padding:10px}
...
.py-upload__name{font-size:13px;font-weight:600;color:#2c46cf;margin:0}
...
.py-modal__textarea:focus{border-color:#3a5bf0}
...
.py-modal__done-icon{width:56px;height:56px;border-radius:50%;background:#f4f6fb;display:flex;align-items:center;justify-content:center;font-size:24px;color:#3a5bf0;font-weight:800;margin:0 auto 16px;border:2px solid #dfe6ff}
...
.py-toast--ok{background:#3a5bf0;color:#fff}.py-toast--err{background:#ef4444;color:#fff}
