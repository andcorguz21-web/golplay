    ...
    <nav style={{
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <Link href="/login" style={{ fontSize: 13, color: 'var(--ink2)', textDecoration: 'none', fontWeight: 500, padding: '8px 14px' }} className="hide-mobile">Ingresar</Link>
          <Link href="/register?type=owner" style={{
            ...
            transition: 'all .15s', boxShadow: '0 2px 14px rgba(58,91,240,.4)',
          }}
      ...
      {mobileOpen && (
        <div style={{ background: '#fff', borderTop: '1px solid var(--line)', padding: '20px 24px 28px' }}>
          {NAV.map(({ label, href }) => (
            <a key={label} href={href} onClick={() => setMobileOpen(false)}
              style={{ display: 'block', fontSize: 16, color: 'var(--ink)', textDecoration: 'none', padding: '12px 0', borderBottom: '1px solid var(--line)' }}>{label}</a>
          ))}
...
function PricingCalculator() {
  ...
  return (
    <div style={{ maxWidth: 960, margin: '32px auto 0', background: '#fff', border: '1px solid rgba(58,91,240,.2)', borderRadius: 22, padding: '28px 36px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
        <span style={{ fontSize: 18 }}>🧮</span>
        <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: 'var(--ink)' }}>Calculá tu costo real por reserva</p>
      </div>

      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 24 }}>
        <div>
          <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 8 }}>Reservas mensuales aproximadas</p>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <input type="range" min={10} max={500} step={5} value={reservas}
              onChange={e => setReservas(Number(e.target.value))}
              style={{ width: 180, accentColor: 'var(--blue)', cursor: 'pointer' }} />
        ...
        <div style={{ display: 'flex', gap: 36, flexWrap: 'wrap' }}>
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 4 }}>Le pagás a GolPlay</p>
            <p style={{ fontSize: 22, fontWeight: 800, color: 'var(--blue)', fontFamily: 'var(--font-d)' }}>{fmtCRC(PLAN_CRC)}</p>
            <p style={{ fontSize: 10, color: 'var(--faint)', marginTop: 2 }}>fijo, sin importar cuántas</p>
          </div>
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 4 }}>Tu costo por reserva</p>
            <p style={{ fontSize: 28, fontWeight: 800, color: 'var(--blue)', fontFamily: 'var(--font-d)', transition: 'all .15s' }}>{fmtCRC(costPerReserva)}</p>
            <p style={{ fontSize: 10, color: 'var(--faint)', marginTop: 2 }}>mientras más reservás, menos pagás</p>
          </div>
        </div>
      </div>

      <p style={{ fontSize: 11.5, color: 'var(--muted)', marginTop: 20, lineHeight: 1.6, borderTop: '1px solid var(--line)', paddingTop: 16 }}>
  ...
  return (
    <section id="canchas" style={{ padding: '80px 0', background: 'var(--dark)', overflow: 'hidden' }}>
      ...
      <div ref={scrollRef} style={{ display: 'flex', gap: 12, overflowX: 'auto', padding: '0 24px 16px', scrollSnapType: 'x mandatory', scrollbarWidth: 'none' }}>
        {images.map((url, i) => (
          <div key={i} style={{ flexShrink: 0, width: 320, height: 200, borderRadius: 16, overflow: 'hidden', scrollSnapAlign: 'start', border: '1px solid var(--line)' }}>
      ...
      {images.length > 3 && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 16 }}>
          <button onClick={() => scrollRef.current?.scrollBy({ left: -340, behavior: 'smooth' })}
            style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--line)', border: '1px solid var(--line)', color: 'var(--ink2)', cursor: 'pointer', fontSize: 16 }}>←</button>
          <button onClick={() => scrollRef.current?.scrollBy({ left: 340, behavior: 'smooth' })}
            style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--line)', border: '1px solid var(--line)', color: 'var(--ink2)', cursor: 'pointer', fontSize: 16 }}>→</button>
        </div>
      )}
    ...
    <>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
          <div className="hero-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
            ...
            <div style={{ animation: 'fadeUp .8s ease both' }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(58,91,240,.12)', border: '1px solid rgba(58,91,240,.3)', borderRadius: 999, padding: '6px 14px', marginBottom: 28 }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--blue)', animation: 'pulse-green 2s infinite' }} />
              ...
              <div className="cta-btns" style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 44 }}>
                <Link href="/register?type=owner" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontFamily: 'var(--font-d)', background: 'var(--g6)', color: '#fff', padding: '15px 28px', borderRadius: 14, textDecoration: 'none', fontSize: 15, fontWeight: 800, boxShadow: '0 4px 32px rgba(58,91,240,.45)', transition: 'all .15s' }}
            ...
            <div className="hide-mobile" style={{ position: 'relative', animation: 'fadeUp .8s ease .2s both' }}>
              <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 24, padding: 24, backdropFilter: 'blur(20px)', boxShadow: '0 40px 80px rgba(20,26,51,.12)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 20 }}>
                  {[
                    { label: 'Reservas hoy', value: '12',      color: 'var(--blue)' },
                    { label: 'Este mes',     value: '₡640K',   color: '#60a5fa' },
                    { label: 'Ocupación',    value: '87%',     color: '#a78bfa' },
                  ].map(k => (
                    <div key={k.label} style={{ background: '#fff', borderRadius: 12, padding: '12px 14px', border: '1px solid var(--line)' }}>
                      <div style={{ fontSize: 10, color: 'var(--muted)', marginBottom: 6 }}>{k.label}</div>
                ...
                <div style={{ background: '#fff', borderRadius: 14, padding: 14, border: '1px solid var(--line)' }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '.06em' }}>Calendario — Hoy</div>
                  {[
              ...
              <div className="hero-card" style={{ position: 'absolute', top: -20, right: -20, background: 'rgba(58,91,240,.15)', border: '1px solid rgba(58,91,240,.3)', borderRadius: 14, padding: '10px 14px', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--blue)', animation: 'pulse-green 2s infinite' }} />
        ...
        <div style={{ display: 'flex', overflow: 'hidden' }}>
          <div className="marquee-track" style={{ display: 'flex', flexShrink: 0 }}>
            {[...COUNTRIES, ...COUNTRIES].map((c, i) => (
              <div key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 28px', borderRight: '1px solid var(--line)', whiteSpace: 'nowrap' }}>
                <span style={{ fontSize: 18 }}>{c.flag}</span>
                <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink2)' }}>{c.name}</span>
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--blue)', background: 'rgba(58,91,240,.1)', padding: '2px 8px', borderRadius: 999 }}>{c.rate}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="pricing-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, maxWidth: 960, margin: '0 auto' }}>
            <FadeIn>
              <div style={{ background: 'rgba(58,91,240,.08)', border: '1px solid rgba(58,91,240,.3)', borderRadius: 24, padding: 40, position: 'relative' }}>
                ...
                <p style={{ fontSize: 15, color: 'var(--muted)', marginBottom: 32 }}>Plan fijo · ≈ $75 USD · 1 mes gratis</p>
                ...
                <Link href="/register?type=owner" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, marginTop: 32, fontFamily: 'var(--font-d)', background: 'var(--g6)', color: '#fff', padding: '14px', borderRadius: 14, textDecoration: 'none', fontSize: 15, fontWeight: 800, boxShadow: '0 4px 28px rgba(58,91,240,.35)', transition: 'all .15s' }}
                  onMouseEnter={e => { e.currentTarget.style.background = 'var(--g7)'; e.currentTarget.style.transform = 'translateY(-1px)' }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'var(--g6)'; e.currentTarget.style.transform = 'none' }}
                >Probá 1 mes gratis <ArrowRight size={16} /></Link>
              </div>
            ...
            <FadeIn delay={120}>
              <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 24, padding: 36 }}>
                <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 20 }}>
                ...
                {COUNTRIES.map(c => (
                  <div key={c.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #fff' }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--blue)', fontFamily: 'var(--font-d)' }}>{c.rate}</span>
                  </div>
                ))}
                <p style={{ fontSize: 11, color: 'var(--faint)', marginTop: 16, lineHeight: 1.6 }}>
            ...
            {BENEFITS.map((b, i) => {
              ...
              return (
                <FadeIn key={b.title} delay={i * 70}>
                  <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 20, padding: 28, transition: 'all .2s' }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(58,91,240,.3)'; e.currentTarget.style.background = 'rgba(58,91,240,.06)'; e.currentTarget.style.transform = 'translateY(-3px)' }}
          ...
          <div className="steps-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 24 }}>
            {STEPS.map((step, i) => (
              <FadeIn key={step.num} delay={i * 100}>
                <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 22, padding: '32px 36px', display: 'flex', gap: 24, alignItems: 'flex-start', transition: 'border-color .2s' }}
                  onMouseEnter={e => (e.currentTarget.style.borderColor = 'rgba(58,91,240,.25)')}
                  onMouseLeave={e => (e.currentTarget.style.borderColor = 'var(--line)')}
                >
                  <div style={{ width: 52, height: 52, borderRadius: 14, flexShrink: 0, background: i === 0 ? 'rgba(58,91,240,.15)' : 'var(--line)', border: `1px solid ${i === 0 ? 'rgba(58,91,240,.3)' : 'var(--line)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <section style={{ padding: '100px 24px', background: '#0f1428' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          ...
          <FadeIn delay={100}>
            <div style={{ position: 'relative', paddingBottom: '56.25%', borderRadius: 20, overflow: 'hidden', background: '#fff', border: '1px solid var(--line)' }}>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <FadeIn>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(58,91,240,.12)', border: '1px solid rgba(58,91,240,.25)', borderRadius: 999, padding: '6px 16px', marginBottom: 28 }}>
              <Zap size={13} color="var(--blue)" fill="var(--blue)" />
            ...
            <p style={{ fontSize: 18, color: 'var(--muted)', marginBottom: 48, lineHeight: 1.7 }}>
            ...
            <div className="cta-btns" style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
              <Link href="/register?type=owner" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, fontFamily: 'var(--font-d)', background: 'var(--g6)', color: '#fff', padding: '17px 36px', borderRadius: 16, textDecoration: 'none', fontSize: 17, fontWeight: 800, boxShadow: '0 4px 48px rgba(58,91,240,.5)', transition: 'all .2s' }}
              ...
              <a href="mailto:hola@golplay.app" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'var(--line)', color: 'var(--ink)', padding: '17px 32px', borderRadius: 16, textDecoration: 'none', fontSize: 17, fontWeight: 600, border: '1px solid var(--line)' }}
                onMouseEnter={e => (e.currentTarget.style.background = 'var(--line)')}
                onMouseLeave={e => (e.currentTarget.style.background = 'var(--line)')}
              >Hablar con el equipo</a>
            </div>

            <p style={{ marginTop: 28, fontSize: 13, color: 'var(--faint)' }}>
      ...
      {/* ── Footer ── */}
      <footer style={{ background: '#030703', borderTop: '1px solid var(--line)', padding: '40px 24px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 20 }}>
          ...
          >Términos</Link>
          <p style={{ fontSize: 12, color: 'var(--faint)' }}>© {new Date().getFullYear()} GolPlay</p>
        </div>
      </footer>

      {/* Mobile sticky CTA */}
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 90, padding: '12px 16px 20px', background: 'linear-gradient(to top, #fff 60%, transparent)', display: 'none' }} className="mobile-cta">
        <Link href="/register?type=owner" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, fontFamily: 'var(--font-d)', background: 'var(--g6)', color: '#fff', padding: '15px', borderRadius: 14, textDecoration: 'none', fontSize: 16, fontWeight: 800, width: '100%', boxShadow: '0 4px 32px rgba(58,91,240,.4)' }}>
