...
.card { background: #fff; border: 1.5px solid var(--line); border-radius: var(--r-xl); padding: 28px; }
...
.input, .textarea {
  ...
  background: #fff; font-family: var(--font-u); font-size: 15px;
  border-radius: var(--r-md); outline: none; color:var(--ink);
  transition: border .15s, box-shadow .15s;
}
.input::placeholder, .textarea::placeholder { color: var(--faint); }
.input:focus, .textarea:focus { border-color: var(--blue); box-shadow: 0 0 0 4px rgba(58,91,240,.12); }
.input:-webkit-autofill, .input:-webkit-autofill:hover, .input:-webkit-autofill:focus {
  -webkit-text-fill-color:var(--ink); -webkit-box-shadow: 0 0 0 1000px #141a33 inset; caret-color:var(--ink);
}
.textarea { resize: vertical; min-height: 90px; font-family: var(--font-u); }
.char-count { font-size: 10px; color: var(--muted); text-align: right; margin-top: 4px; letter-spacing: .04em; }
.file-row { display: flex; gap: 14px; align-items: center; flex-wrap: wrap; }
.file-button {
  display: inline-flex; align-items: center; gap: 8px;
  padding: 11px 18px; border: 1.5px dashed var(--line); background: #fff;
...
.file-button:hover { border-color: var(--blue); background: rgba(58,91,240,.08); color: var(--blue); }
.logo-preview { width: 80px; height: 80px; border-radius: 14px; object-fit: cover; border: 1.5px solid var(--line); }
.helper { font-size: 12px; color: #9ca3af; margin-top: 8px; line-height: 1.5; }
.submit {
  width: 100%; padding: 15px; margin-top: 8px;
  background: linear-gradient(135deg, var(--blue), var(--g6));
  color:var(--ink); border: none; cursor: pointer;
  font-family: var(--font-d); font-size: 15px; font-weight: 800;
  border-radius: var(--r-md);
  display: flex; align-items: center; justify-content: center; gap: 7px;
  box-shadow: 0 4px 22px rgba(58,91,240,.3); transition: all .15s ease; letter-spacing: -.01em;
}
.submit:hover { transform: translateY(-1px); box-shadow: 0 6px 28px rgba(58,91,240,.42); }
...
.warn-cta {
  display: inline-flex; align-items: center; gap: 6px;
  padding: 12px 22px; background: var(--g6); color:var(--ink);
  border-radius: var(--r-md); font-weight: 700; font-size: 13px; text-decoration: none;
  box-shadow: 0 2px 10px rgba(58,91,240,.28);
}
