...
type TeamRow = {
  ...
  description: string | null; captain_player_id: string;
  created_at: string; role: "captain" | "member"; joined_at: string;
};
...
.empty {
  ...
  background: var(--white); border: 1.5px dashed var(--bd); border-radius: var(--r-xl);
}
...
.warn-card {
  ...
  padding: 36px 28px; text-align: center; max-width: 480px; margin: 12px auto;
...
.skeleton-card {
  height: 100px; border: 1.5px solid var(--bd); border-radius: var(--r-lg);
  background: var(--white); padding: 18px; display: flex; gap: 14px; align-items: center;
}
`;
      ...
      <div className="page">
        ...
        <div className="content-section">
          ...
          {state === "ready" && teams.length > 0 && (
            <div className="grid">
              {teams.map((t, i) => (
                <Link key={t.id} href={"/equipos/" + t.slug} className="team-card" style={{ animationDelay: `${i * 60}ms` }}>
                  ...
                  <div className="card-body">
                    ...
                    <div className="card-meta">
                      <span className={"role-pill " + t.role}>{t.role === "captain" ? "Capitán" : "Miembro"}</span>
