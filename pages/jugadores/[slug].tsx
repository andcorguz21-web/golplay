...
export default function PerfilPublico({ player }: { player: Player | null }) {
  ...
  useEffect(() => {
    ...
    if (DeviceEvt && typeof DeviceEvt.requestPermission === "function") {
      // iOS 13+: requiere permission tras user gesture
      const wrap = wrapRef.current;
    ...
    <>
      ...
      <div className="page">
        <header className="header">
          <button className="btn-back" onClick={() => router.back()} aria-label="Volver">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
              <path d="M19 12H5M12 19l-7-7 7-7" />
            </svg>
          </button>
          <Link href="/" className="logo-link">
            <Image src="/logo-golplay.svg" alt="GolPlay" width={110} height={80} style={{ objectFit: "contain" }} />
          </Link>
...
.page { min-height: 100vh; background: #0C0D0B; font-family: 'Inter', system-ui, -apple-system, sans-serif; color: #e2e8e0; }
.header { display: flex; align-items: center; justify-content: space-between; padding: 16px 24px; border-bottom: 1px solid rgba(255,255,255,.06); }
.btn-back { width: 40px; height: 40px; border-radius: 12px; background: rgba(255,255,255,.06); border: 1px solid rgba(255,255,255,.08); color: #e2e8e0; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: background .15s; }
